...
// Design Name: 
// Module Name: Float_multiplication_tb
...
module Float_multiplication_tb(); 

    logic clk = 0, reset_n = 0, enable = 0, data_ready; 
    float32 idata, odata, expected; 
    Float_multiplication multiplier(clk, idata, odata, data_ready, reset_n, enable); 
...
//                        Only dispays the failed tests
                        $display("%s: %d Multiplication of %f and %f resulting in %f instead of %f with an error of %f", run_failed?"FAIL":"PASS", i, 
